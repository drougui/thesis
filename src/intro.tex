...
because of the prohibitive computation time and/or memory needed for this task.
Second we presented 
the difficulty of defining 
the probability distributions of a POMDP 
encoding a given problem 
(e.g. defining the observation function 
when using computer vision algorithms).
Finally the problem of managing 
the knowledge of the agent 
has been discussed:
representing the initial agent's 
lack of knowledge 
...
of computing appropriate strategies for partially observable domains.
The computed strategies have to make 
the robot fulfill the mission
as well as possible, and from the very first execution,
\textit{i.e.} strategy computations are performed 
before any real execution of the mission.
For instance, RL algorithms may improve, 
at each execution of a given robotic mission,
the prior strategies that we propose to compute in this work:
this interesting question is not considered here.
%to this purpose, a criterion
%(e.g the discounted expected total reward of Equation \ref{criterion})
%is maximized.
The general challenge of our work 
...
and the management of the knowledge of the agent.

Although the use of a more expressive framework 
may make strategy computations harder, 
previous discussions about the parameter imprecision issue
and about the agent knowledge issue 
...
this theory encodes uncertainty of a given situation
with a mass $m$ defined on each disjunctive subsets 
of the finite universe $\Omega$:
$m : 2^{\Omega} \rightarrow [0,1]$, 
with $\sum_{A \subseteq \Omega} m(A) = 1$.
...
of times the considered event occurred 
in a sufficiently big dataset of events, 	
over the size of the used dataset. %%% TODO ensuite c'est pas beau!!!
Suppose now that 
we need to compute,
...
Consider now that some of 
the elementary events 
$\omega \in \Omega$
considered in the problem,
have been imprecisely observed in practice. %% TODO 
Usually, instances of the elementary event 
are included several times in the dataset,
the number of instances of this elementary event 
leading to the frequentist probability distribution
after the normalization.
In the case of imprecise observation %%%TODO PAS OBSERVATION 
during the dataset generation,
a disjunctive set of events is returned
instead of the actual elementary event:
indeed, this set describes the actual imprecision 
of the observation,
since the actual elementary event has not been
correctly  
