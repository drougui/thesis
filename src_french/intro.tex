...
Cependant le comportement du système robotique
peut être étudié en effectuant de nombreux tests 
sur lui \textit{i.e.} des exécutions d'actions, 
ou en utilisant les connaissances d'un expert de ce système.
De même, 
...
Des relations peuvent aussi être obtenues 
entre les observations, 
les états du système et les actions  
à l'aide de tests des capteurs dans de nombreuses situations, 
de la description technique de ces même capteurs,
du traitement des données utilisé, 
ou de n'importe quelle information experte.
Par exemple, 
...
puisque elle est à la fois le résultat 
d'un traitement des données des capteurs,
et l'information sur laquelle se base le modèle de décision:
ici, les données sont les images provenant de la camera.
Pour une camera donnée, et un algorithme de vision donné,
le comportement de l'observation
est lié à l'action
et à l'état du système
lors du processus de prise d'image.
...
que dans la mission considérée,
le robot a une connaissance parfaite
des caractéristiques d'intérêt du problème 
via ses capteurs.
Dans ce modèle,
...
qui sont notées $a \in \mathcal{A}$.
...
La mission de l'agent est décrite en termes de \textit{récompenses}.
...
une mission est considérée bien remplie
...
en tenant compte du comportement probabiliste des états du système
...
\node (bli) at (0,6.5) {};
\node (pomdp3) at (4.2,6) {\color{orange}{$s \in \mathcal{S}$}: \color{black}{\textbf{état du système}}};
\node (t1) at (2,6) {};
\node (r1) at (-2.3,5.5) {};
\node (r11) at (1,4.5) {};
\draw[->,>=latex,color=orange!60,line width=1mm] (t1) to (r1);
\draw[->,>=latex,color=orange!60,line width=1mm] (t1) to (r11);
\node (pomdp4) at (5.8,4.5) {\color{blue!60}{$o \in \mathcal{O}$:} \color{black} \textbf{observations du système}};
\node (t2) at (3.3,4.4) {};
\node (r2) at (-2.3,5.05) {};
\draw[->,>=latex,color=blue!40,line width=1mm] (t2) to[bend left] (r2);
\node (pomdp5) at (2,3) {\color{red}{$a \in \mathcal{A}$:} \color{black} \textbf{actions de l'agent}};
\node (t3) at (-0.3,3) {};
\node (r3) at (-2,4.2) {};
...
la position du feu et son niveau sur une échelle de ``petit feu'' à ``feu important'', etc.
...
et qui renvoie une estimation du niveau ou de la position du feu,
peut être modélisée par une observation.
...
le but du robot est donc d'éteindre les incendies sans gaspiller de l'eau.
...
étant donnée la position courante,
...
cette influence est définie par la \textbf{fonction d'observation} 
$\textbf{p} \paren{o' \sachant s',a}$, où $o' \in \mathcal{O}$ est l'observation: 
...
conditionnellement à l'ensemble des observations et des actions successives depuis le début du processus:
...
tels que celui basé sur la somme des récompenses actualisées:
...
Ainsi, un modèle plus flexible a été construit:
il tient compte de \textit{l'observation partielle} du système par l'agent.


%%% POMDP
Les PDM Partiellement Observables 
...
restent les mêmes que pour la définition des PDM.
...
Résoudre un PDMPO consiste à calculer une stratégie 
...
un critère courant pour la stratégie est
...
\end{tikzpicture}
\caption[Réseau Bayésien illustrant la mise à jour de l'état de croyance]{Réseau Bayésien illustrant la mise à jour de l'état de croyance: 
les états sont représentés par des ronds gris, 
l'action est représentée par le losange rouge,
...
est l'estimation probabiliste de l'état courant (resp. suivant) du système, 
$s_t$ (resp. $s_{t+1}$).}
\label{BayesNetPOMDP}
\end{figure}%
...
les fonctions $d$ définies sur $\mathbb{P}^{\mathcal{S}}$ 
telles que les $d_t = d(b_t) \in \mathcal{A}$ successifs 
maximisent l'espérance des récompenses (\ref{criterion}):
les décisions de l'agent sont alors basées sur l'état de croyance.
...
Ils sont assez fréquemment utilisés en robotique
...
Pour cela, il utilise ces données, appelées alors observations.
...
pour la spécification explicite d'une telle stratégie.
...
Cependant, pour des problèmes non triviaux, seules des solutions approchées peuvent être calculées,
...
ou bien les approches de Monte Carlo \cite{NIPS2010_4031},
...
et utilise un classifieur (\textit{classifier}) calculé à partir d'une base de données d'images
(comme \textit{NORB}, \textit{cf.} figure \ref{NORB}).
Le classifieur est généré avant la mission (hors-ligne, \textit{off-line})
avec une base de données d'images
(\textit{cf.} partie droite de l'illustration, \textit{dataset}), 
et la sortie du classifieur 
est utilisée lors de la mission (en ligne, \textit{online}) 
comme une observation pour l'agent (\textit{cf.} partie gauche de l'illustration).
...
et dont les observations proviennent d'algorithmes de vision
basés sur de l'apprentissage statistique.
(\textit{cf.} figure \ref{observation_robot}).
Dans cette situation, le robot utilise un \textit{classifieur}
pour reconnaître les objets dans les images:
...
(\textit{cf.} matrice de confusion de la figure \ref{confusion_matrix}).

Le classifieur est calculé en utilisant une \textit{base de données d'entraînement} 
(comme \textit{NORB}, \textit{cf.} figure \ref{NORB}, 
...
d'animaux (\textit{animals}), voitures (\textit{cars}), humains (\textit{humans}), avions (\textit{planes}) ou camions (\textit{trucks}): 
...
base de données d'images \textit{NORB}: \color{blue} $(\mbox{image}_i,\mbox{étiquette}_i)_{i=1}^N$
};
...
\end{tikzpicture}
\caption[Exemple de base de données pour la vision artificielle]{Exemple de base de données pour la vision artificielle: 
la base de données d'images étiquetées \textit{NORB} \cite{LeCun.2004},
destinée à l'apprentissage statistique.
La taille de \textit{NORB} est supérieure à $3.10^5$,
et les images de cette base de données représentent des objets de ces $5$ classes:
``animal'', ``car'', ``human'', ``nothing'', ``plane'' et ``truck''.
Chaque élément d'une base de données d'images est composé d'une image (par exemple une image montrant une voiture)
...
(\textit{i.e.} sa fréquence de bonne prédiction) 
est inévitablement dépendant de la base de données.
Cela pose un pose un problème si la variabilité de la base de donnée
est trop faible:
dans ce cas, le comportement probabiliste du classifieur
sera dépendant de ces images en particulier
et le système robotique aura des mauvaises capacités d'observation
...
(par exemple \textit{NORB}, figure \ref{NORB}, bien que la variabilité pourrait être idéalement supérieure):
notons cependant qu'avec ces bases de données,
la performance de vision est réduite,
...
en utilisant une telle base de données étiquetée.
Elle doit être différente de celle
utilisée pour l'entraînement
et est appelée \textit{base de données de test}:
la fréquence des observations peut être déduit de cette matrice,
en normalisant les lignes ce qui en fait des distributions de probabilité.
Une ligne correspond à un objet de la scène
et les probabilités de cette ligne sont les probabilités d'observation,
\textit{i.e.} chaque valeur de probabilité est la fréquence avec laquelle
le classifieur renvoie le nom de l'objet de la colonne correspondante.
Ici le classifieur a été calculé à partir d'un Réseau Convolutionnel \cite{Lecun98gradient-basedlearning}.
...
Cette approche soulève encore le problème de la représentativité
de la base de données pour la mission voulue.
Si la base de donnée de test n'est pas représentative,
ces probabilités d'observation risquent de ne pas être fiables,
...
Finalement, si les bases de données considérées sont étiquetées plus précisément,
(comme \textit{NORB}, qui inclut des informations telles que la luminosité, ou l'échelle de l'objet),
nous pouvons imaginer que les probabilités d'observation calculées (à partir de la matrice de confusion)
seraient plus fiables, ou la performance de vision améliorée
(puisque la séparation demandée au classifieur 
est plus simple avec cette précision).
Cependant, comme cela implique l'utilisation de 
plus d'observations ou d'états, le PDMPO est plus dur à résoudre.

En conclusion, l'utilisation du modèle PDMPO fait l'hypothèse
...
doit être prise en compte pour rendre le robot autonome en toutes circonstances.
...
Exemple d'une matrice de confusion en classification multi-classes:
cette matrice est calculée avec une base de données de test,
différente de la base de données d'apprentissage.
Chaque ligne ne considère que les images d'un certain objet,
et les nombres représentent les réponses du classifieur:
par exemple, $3688$ images d'animaux sont bien reconnues, 
mais $575$ sont confondues avec un humain.
La proportion de réponses correctes pour cet objet est $80.223\%$.
L'environnement Torch7 \cite{Collobert_NIPSWORKSHOP_2011}
a été utilisé pour obtenir un classifieur, 
et pour calculer cette matrice à partir du classifieur
...
sont appelées \textit{paramètres} du PDMPO, 
ou encore \textit{paramètres} du modèle. 
A notre connaissance,
le premier modèle construit dans le but de gérer l'imprécision
des paramètres est nommé PDMPOPI, 
pour \textit{PDMPO à Paramètres Imprécis} \cite{Itoh2007453}. 
...
est décrit à l'aide d'une borne supérieure et inférieure
...
la flexibilité amenée par l'imprécision des paramètres
...
puisque le critère utilisé est une borne inférieure
du critère désiré.
...
mène à l'utilisation de nombreuses approximations
...
peut être mentionné: 
il concerne la définition de l'état de croyance
...
sur l'état du système, prend part dans la définition
du PDMPO. 
...
Même s'il est très peu probable que ce soit le cas,
...
ne peut pas le mettre à jour correctement
...
(\textit{i.e.} sur toutes les positions possibles du robot/cible, 
ou sur toutes les natures possibles de cible), 
mais cette réponse provient de l'interprétation subjective
des probabilités \cite{de1974theory,Dubois96representingpartial}.
En effet, les probabilités sont les mêmes
puisqu'aucun événement n'est plus plausible qu'un autre:
...
les fonctions de transition et observation,
avec cet état de croyance initial qui est une distribution de probabilité subjective:
cela n'a pas toujours de sens, et dans notre cas cette approche est douteuse.
...
dont le calcul d'une stratégie 
suffisamment proche d'une stratégie optimale
est impossible, 
du fait d'un trop grand temps de calcul
ou d'un manque de mémoire vive.
...
Ces problèmes forment le point de départ de notre travail.
...
aux calculs très complexes et aux paramètres difficiles à définir.
...
semble pouvoir répondre aux problèmes soulevés.
...
est bénéfique en matière de complexité
et de modélisation.
...
L'opérateur de marginalisation est l'opérateur maximum ($\max$):
étant donné une distribution de possibilité jointe 
$\pi: \mathcal{S} \times \mathcal{O} \rightarrow \mathcal{L}$,
la distribution de possibilité marginale sur $\mathcal{S}$ est
$\pi(s) = \max_{o \in \mathcal{O}} \pi(s,o)$.
De plus, l'opérateur minimum ($\min$) est utilisé pour calculer 
...
Ainsi, il est possible d'apprivoiser la théorie des possibilités
...
a été proposée dans \cite{Sabbadin1999pipomdp}:
...
au lieu de distributions de probabilité.
...
chaque étape de temps mène à un nombre fini d'états de croyance suivants,
donc cet espace est dénombrable.
Dans le but d'obtenir des propriétés utiles
et des moyens de calculer une stratégie,
...
En effet, le nombre de distributions de possibilité qualitatives
sur les états du système $\mathcal{S}$
est inférieur à $\# \mathcal{L}^{\# \mathcal{S}}$,
...
tout $\pi$-PDMPO se réduit à un $\pi$-PDM 
...
est plus faible que la complexité d'un PDM probabiliste, 
qui est polynomiale \cite{Papadimitriou1987}:
la complexité d'un $\pi$-PDMPO est
...
les fonctions d'observation probabilistes:
les probabilités 
des réponses
des algorithmes de vision
dans le contexte d'une mission robotique
sont mal connues et difficiles à définir en pratique.
Il est plus facile de trouver des estimations qualitatives
des performances de reconnaissance de l'algorithme:
le modèle $\pi$-PDMPO
ne requiert que des données qualitatives, 
donc il permet de construire un modèle
sans l'utilisation d'informations autres que
...
tenant compte uniquement 
de la manière dont les fréquences
de réponses sont classées:
en présence d'un humain (\textit{i.e.} deuxième ligne),
la réponse la plus fréquente est ``\textit{human}'',
la deuxième réponse est ``\textit{nothing}'',
la troisième est ``\textit{car}'', etc.
Donc, la distribution de possibilité correspondante
est telle que,
conditionnellement à la présence d'un humain, 
le degré de possibilité de la réponse ``\textit{human}''
est plus grand que le degré de possibilité de la réponse ``\textit{nothing}'',
qui est plus grand que le degré de possibilité
de la réponse ``\textit{car}'', etc.
Au lieu d'attribuer des fréquences
qui ne sont pas vraiment
fiable en pratique,
le modèle possibiliste qualitatif
exprime naturellement ces imprécisions relatives au problème.
...
cette distribution peut être utilisée
...
L'utilisation de la théorie des possibilités qualitatives \cite{DuboisPS01}
...
ainsi cette théorie offre des solutions aux
trois problèmes mis en évidence précédemment.
Cependant, notons qu'un cadre qualitatif
...
\section*{Description de l'\'Etude}
...
en matière de simplification des calculs et de modélisation. 
...
décrit la structure de cette thèse.
En effet, chacune des sections suivantes
correspond à un chapitre de notre travail,
...
seul un article de dix pages a déjà traité des $\pi$-PDMPOs.
...
dans laquelle quelques variables d'état sont complètement observables. 
Elle est appelée $\pi$-PDMOM et généralise à la fois les $\pi$-PDM et les $\pi$-PDMPO.
Cette contribution réduit considérablement 
la complexité de résolution des $\pi$-PDMPO, 
...
Plus généralement, il est très courant de manipuler 
des variables visibles en robotique \cite{OngShaoHsuWee-IJRR10}.
...
les résultats expérimentaux comparent
...
de l'état de croyance possibiliste qualitatif
dans certaines situations détaillées ensuite.
Les principales contributions de ce chapitre
ont été publiées dans \cite{Drougard13}.
Il est à souligner que les résultats expérimentaux
...
permettent la résolution de problèmes de planification
structurés
dont l'espace des états est plus grand.
...
\textit{SPUDD} \cite{Hoey99spuddstochastic},
nous avons conçu un algorithme nommé \textit{PPUDD}
pour résoudre les $\pi$-PDMOM factorisés 
en utilisant des \textit{arbres de décision algébriques} (\textit{ADDs}).
L'intuition motivant cette contribution,
est que le calcul entre \textit{ADDs} 
...
devraient mener à des \textit{ADDs} plus petits
puisque la somme et le produit
sont remplacés par les opérateurs $\min$ et $\max$.
Ces derniers produisent des \textit{ADDs}
...
représentant les états de croyance successifs.
De plus, les variables définissant un
$\pi$-PDMOM sont celles représentant
les états du système et les observations.
C'est pourquoi la section de ce chapitre
qui suit propose des conditions suffisantes
...
Les performances de notre algorithme \textit{PPUDD}
sont ensuite comparées à celles des homologues probabilistes,
en termes de temps de calcul
et avec des critères mesurant 
la réussite de la mission.
Enfin, la dernière partie de ce chapitre
décrit les résultats de \textit{PPUDD}
...
dans le but de tester les performances de \textit{PPUDD}
contre celles des algorithmes probabilistes,
en termes d'espérances de la somme des récompenses,
et sur de nombreux problèmes de planification.
...
certaines failles de notre algorithme,
...
algorithmes utilisant des \textit{ADDs} (notamment sont homologue direct, \textit{SPUDD}),
...
L'approche proposée dans \cite{DrougardDFT15}
