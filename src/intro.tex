...
\section*{Context}
%%% CONTEXT
\malettrine{P}{roviding} autonomy to a robot 
...
depend on the robot actions and system states. 
...
makes a step further into the modeling flexibility, 
...
This imprecision results in measures
...
as explained in Section \ref{section_piPOMDP} of Chapter \ref{SOTA}, 
any $\pi$-POMDP reduces to a $\pi$-MDP whose the system space is
the qualitative possibilistic belief space,
...
at worst exponential the process description,
...
the $\pi$-POMDP model 
only require qualitative data, 
thus it allows to build the model 
without the use of more information 
...
expresses naturally imprecision 
about the problem.
...
and models formally the lack of knowledge:
thus this theory offers solutions to
the three previously highlighted issues.
However we note that, as a qualitative framework, 
it does not allow frequentist information encoding.

%%%NOT WIDELY STUDIED TODO TODO TODO
To the best of our knowledge,
a very limited study of the $\pi$-POMDP model
is present in the literature up to now:
...
including algorithms used in the next chapters. 
...
only one ten-pages paper has already dealt with
the $\pi$-POMDPs: 
...
Moreover, the choice of dedicating this first chapter 
...
%% INTRO
The size of the robotic problem dealt in the previous chapter
...
are meant to make possible computations for
larger structured planning problems. 

%% PPUDD and factored models
The first part of this chapter 
proposes a definition 
for the \textit{factored $\pi$-MOMDPs}:
these processes benefit from additional
...
The possibilistic qualitative framework is shown
to be more 
This Profitant \`a la fois du caract\`ere qualitatif de ces processus,
ainsi que de la structure des probl\`emes en pratique, 
Computation time is decreased .

%% belief factorization
et d\'ecrivons sur les hypoth\`eses d'ind\'ependance
possibles pour de tels processus. independent BELIEFs, pearl!!
...
A new translation from Partially Observable MDP into Fully
Observable MDP is described here. Unlike the classical translation, the
resulting problem state space is finite, making MDP solvers able to solve
this simplified version of the initial partially observable problem: this
approach encodes agent beliefs with possibility distributions over states,
leading to an MDP whose state space is a finite set of epistemic states.
