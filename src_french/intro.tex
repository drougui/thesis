...
\end{tikzpicture}
\caption[Réseau Bayesien illustrant la mise à jour de l'état de croyance]{Bayesian Network illustrating the belief update: the states are the gray circular nodes, 
...
is PSPACE-hard in finite horizon \cite{Papadimitriou1987} 
and even
undecidable in infinite horizon \cite{Madani1999UPP315149.315395}.
Moreover a space exponential in the problem description may be required 
for an explicit specification of such a strategy.
See \cite{Mundhenk2000CPP867838} 
...
can be is called Convolutional Nextwork \cite{Lecun98gradient-basedlearning}.
Figures (\ref{NORB}), (\ref{confusion_matrix})
...
\end{tikzpicture}
\caption[Exemple de base de donnée pour la vision artificielle]{Example of image dataset for computer vision: 
...
This dataset can be used for supervised learning to compute a classifier.
...
preprocessing followed by classifiers etc.) 
...
and is thus named 
\textit{robust POMDP} \cite{Osogami15}.
Inspired by the corresponding work 
in the fully observable case 
(called \textit{uncertain MDP} \cite{NE05}),
...
such as the convexity of credal sets \cite{Osogami15}.
...
this thesis does not deal with Reinforcement Learning (RL) \cite{NIPS2009_3780,Ross2011BAL1953048.2021055}.
...
\end{tikzpicture}
\caption[Théories de l'incertain les plus connues, et leurs relations]{Most known uncertainty theories and their relations. 
...
\subsection*{Théorie Alternatives de l'Incertitude}
...
\node at (0,-1) {};
\end{tikzpicture}
\caption[Ensembles focaux d'une mesure de probabilité et d'une mesure de possibilité]
...
from Qualitative Possibility Theory \cite{DuboisPS01}.
