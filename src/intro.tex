...
the universe $\Omega$ can be defined as
the set of system states $\mathcal{S}$,
...
In this case, the universe $\Omega$ can be defined as the set of observations $\mathcal{O}$,
...
All subsets of the universe $\Omega$ 
...
Properties of this measure lead to
...
(same meaning as $\textbf{p}(\omega)=0$, with $\textbf{p}:\Omega \rightarrow [0,1]$ a probability distribution), 
Moreover, elementary events 
such that $\pi(\omega)=1$ 
are entirely possible
(but not necessary the most probable one).
If the current possibilistic belief 
coincide with the distribution
$\pi(\omega)=1$ $\forall \omega \in \Omega$, 
all elementary events are totally possible,
and it models therefore a total ignorance 
of the actual elementary event:
Possibility Theory can thus encode agent ignorance.



The full knowledge of the current state, 
say $\tilde{s} \in \mathcal{S}$, 
is encoded by a possibility distribution equal to the classical indicator
function of elementary event $\pi(s) = \mathds{1}_{\set{ s=\tilde{s} }}(s)$.
