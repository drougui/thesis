...
pour cette raison, les $\pi$-PDMPO peuvent être vus comme un modèle plus simple
...
\section{Observabilité Mixte et $\pi$-PDMOM}
\label{sectionpiMO}
...
l'état du système est décrit pas la variable $S_t = (S_{v,t},S_{h,t})$.
L'observation reçue est $O_t=(O_{v,t},O_{h,t})$
avec $O_{v,t}=S_{v,t}$, 
et $O_{h,t}$ dépend de on $S_t$ et de l'action $a_t$.}
\label{piMOMDP}
\end{figure}
La résolution d'un $\pi$-POMDP se confronte au fait que
...
un modèle structué qui prend en compte
les situations dans lesquelles l'agent observe directement un partie de l'état du système. 
Un $\pi$-POMDP qui modélise une telle situation respecte la \textit{propriété d'observabilité mixte}.
Les états de croyance ne sont alors utilisé que
pour les composantes partiellement observables
et la taille de l'espace d'état est significativement résuit.
...
peut être écrit comme un produit cartésien
...
l'agent observe \textit{nécessairement} $o_v=s_v$ (if $o_v' \neq s_v$, $\pi_t \paren{ o_v' \sachant s_v } = 0$).
Formellement, vu comme un $\pi$-PDMOM, 
...
\end{array} \right.
\end{eqnarray}
puisque $\pi_t \paren{ o_v' \sachant s_v'} = 1$ si $s_v'=o_v'$ et $0$ sinon. 
Le théorem suivant, basé sur cette égalité,
permet de définir les états de croyance sur les états du système cachés.
...
n'importe $\beta \in \Pi^{\mathcal{S}_h}_{\mathcal{L}}$
...
lorsque le $\pi$-PDMOM satisfait la propriété d'observabilité mixte
le théorème suivant réécrit la mise à jour de l'état de croyance
pour 
$\beta_h \in \Pi^{\mathcal{S}}_{\mathcal{L}}$,
l'état de croyance sur les états cachés du système 
$s_h \in \mathcal{S}_h$.
\begin{theorem}[Mise à jour de la croyance pour un $\pi$-PDMOM]
\label{thmMomdpBelup}
Si un problème peut se modéliser par un $\pi$-MOMDP  
...
l'état de croyance courante sur l'état caché est 
...
et les objets visibles (état visible du système state et observation) $s_v' \in \mathcal{S}_v$ et $o_h' \in \mathcal{O}_h$.
La mise à jour de l'état de croyance est noté \[ \beta_h' = \nu_h \paren{ s_v, \beta_h, a, s_v', o_h' }. \]
\end{theorem}
...
où $\pi_t \paren{ s_v',o_h' \sachant s_v, \beta_h, a }$ est défini juste au-dessus,
...
nous proposons ici le premier algorithme d'itérations sur les valeurs
pour $\pi$-PDM,
qui renvoie une stratégie optimale.
Comme mentionné dans \cite{Sabbadin1999pipomdp}, 
nous faisons l'hypothèse de l'existence d'une action
``rester'', notée $\widehat{a}$, 
qui retient le système dans sont état courant 
avec nécessité $1$. 
...
Notons que une hypothèse similaire est utilisée
...
\textit{i.e.} qui maximise le critèren (\ref{optcriterion}),
...
celà signifir que $s$ est un but ($\Psi(s)>0$)
...
et $D$ une constante de normalisation.
...
Ici, $\mathcal{L}$ sera un sous ensemble fini de $[0,1]$,
c'est pourquoi $1_{\mathcal{L}}$ peut être noté $1$. 
...
avec une transformation probabilité-possibilité \cite{Dubois93onpossibility/probability} 
aurait été équivalente. 
La distribution de préférence $\mu$ est égale à $0$ pour tous les états du système,
et à $1$ pour les états $[(x_{1},y_{1}),A_1]$ et $[(x_{2},y_{2}),A_2]$ où $(x_{i},y_{i})$ 
est la position de la cible $i$. 
Comme mentionné dans \cite{Sa1999.5}, 
...
$\# \mathcal{B}^{\pi} = \mathcal{L}^{\# \mathcal{S}} - (\mathcal{L}-1)^{\# \mathcal{S}} = 5^{18} -4^{18} \geqslant 3.7.10^{12} $ 
au lieu de $81$ états
avec un $\pi$-PDMOM. 
...
est possible à l'aide des espérances de la somme des récompenses 
...
\mbox{c-à-d} si quel que soit $i \in \{1,2\}$, $\sqrt{(x-x_i)^2+(y-y_i)^2}>C$, 
avec $C$ une constante positive, alors
$Pr \paren{good_i \sachant x,y }$ 
$= 1-P_{bad} < \frac{1}{2}$.  
Dans tous les autres cas, 
le modèle probabiliste est effectivement le bon.
...
C'est dû au fait que la mise à jour possibiliste de l'état de croyance 
...
(le minimum entre la distribution de possibilité sur l'état du système, \mbox{c-à-d} l'état de croyance, 
...
\begin{itemize}
\item elle est encore plus sceptique à propos de $s_{h}^1$ 
...
\item elle devient l'état de croyance opposé si l'observation est plus fiable et contredit l'état de croyance précédent 
($\pi \paren{o_h \sachant s_v,s_h^2,a }$ est à la fois plus petit que $\beta_{h}(s_{h}^1)$ et que $\pi \paren{o_h \sachant s_v,s_h^1,a }$); 
\item elle reste simplement la même
...
Classiquement un état de croyance $\beta_1 \in \mathcal{B}^{\pi}$ 
est dit plus spécifique qu'un état de croyance $\beta_2 \in \mathcal{B}^{\pi}$
si pour chaque $ s \in \mathcal{S}$, $\beta_1(s) \leqslant \beta_2(s)$.
Afin d'avoir un ordre total sur $\mathcal{B}^{\pi}$, 
l'échelle $\mathcal{L}$ est considérée ici 
comme un sous ensemble de $[0,1] \subset \mathbb{R}$.
La relation d'ordre $\preceq$ sur $\mathcal{B}^{\pi}$ peut alors être définie 
pour classer les état de croyance selon leur spécificité~:
...
\begin{theorem}
Soit $\beta_0 \in \mathcal{B}^{\pi}$ l'état de croyance initial 
modélisant l'ignorance totale, \mbox{c-à-d} pour tous les $ s \in \mathcal{S}$, $\beta_0(s)=1$.
Si la fonction de transition $T^{\pi}$ est déterministe,
et si les observations ne sont pas informatives, \mbox{c-à-d} 
 $\forall s' \in \mathcal{S}$ , $\forall a \in \mathcal{A}$, $\forall o' \in \mathcal{O}$, $\Omega^{\pi} \paren{s',a, o'}=1$,
alors si l'état de croyance $\beta_{t+1} \in \mathcal{B}^{\pi}$ 
est le résultat de la mise à jour de l'état de croyance 
$\beta_t \in \mathcal{B}^{\pi}$, $\beta_{t+1} \preceq \beta_{t}$.
Ce résultat reste valable si pour chaque action $T^{\pi}$ est l'identité
et $\forall (o',\tilde{o}) \in \mathcal{O}^2$, $\forall (a,\overline{a}) \in \mathcal{A}^2$ et
$\forall s' \neq  \tilde{s} \in \mathcal{S}$ t.q. $\Omega^{\pi} \paren{s',a, o'} < 1_{\mathcal{L}}$,
$\Omega^{\pi} \paren{s',a, o'} \neq \Omega^{\pi} \paren{ \tilde{s}, \overline{a}, \tilde{o} }$.
...
ou d'ignorer les observation moins fiables~: 
...
Il perd donc beaucoup de temps à sortir de cette boucle. 
...
vers la mauvaise cible (\mbox{c-à-d} l'agent pense fortement que la cible $1$ est $B$ alors que c'est $A$ en réalité). 
...
Nous avons proposé un algorithme d'Itération dans l'espace des Revenus (IR) pour les PDM possibilistes.
Celui-ci calcule une stratégie optimale stationnaire pour un horizon infini
...
Enfin, cet algorithme a été étendu 
au nouveau modèle des PDM possibilistes qualitatifs à Observabilité Mixte, 
dont la complexité est exponentiellement plus petite que celle des PDMPO
...
Le cadre possibiliste qualitatif peut cependant être inadapté
aux situations dans lesquelles quelques informations probabilistes 
sont disponibles à propos du problème~: 
les PDMPO à Paramètres Imprécis \cite{Itoh2007453} 
et les PDMPO à Paramètres Bornés \cite{NiYaLiaZhi} 
intègrent la méconnaissance en considérant 
des ensembles de distributions de probabilité,
plutôt qu'une seule. 
Lorsque de tels ensembles ne peuvent pas être déterminés,
ou bien lorsqu'un modèle qualitatif suffit, les $\pi$-PDMPO 
peuvent être une bonne alternative~:
les PDMPOPI et les PDMPOPB sont très difficiles à résoudre en pratique.

La version pessimiste des $\pi$-PDM peut aussi être construite,
mais l'optimalité de la stratégie renvoyée par l'algorithme d'IR associé
semble dure à prouver, 
essentiellement car il n'est pas suffisant de construire
une trajectoire maximisante, 
comme fait dans la preuve en \ref{proofThmIV}. 
Les travaux \cite{LIP61498} et \cite{0024909} 
peuvent être utiles pour obtenir des résultats
à propos des $\pi$-PDM pessimistes,
...
Certaines conditions suffisantes
...
pour obtenir un meilleur moyen 
d'avoir une idée de son comportement
