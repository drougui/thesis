...
As an idea to make the robot more powerful
in the long term, RL algorithms may improve
the prior strategies 
that we propose 
to compute in this work,
taking into account each past executions 
of the robotic mission.
However this idea 
is not considered 
here since we focus 
on the aim of making the robotic mission 
as fulfilled as possible 
also during 
the first executions of the process,
and thus without the knowledge 
of a sufficient amount of
records of past executions
...
making if possible the strategy computation easier,
...
a finite set whose elements represent 
...
on each disjunctive subsets 
of the universe 
\textit{i.e.} $m : 2^{\Omega} \rightarrow [0,1]$, 
and sum to $1$ over these subsets: $\sum_{A \subseteq \Omega} m(A) = 1$.
...
of recorded samples,
over the size of the used dataset 
...
the universe $\Omega$ can be defined as $\mathcal{S}$,
and then an elementary event 
is a system state $s' \in \mathcal{S}$.
An other example is the computation 
...
In this case, the universe can be defined as $\mathcal{O}$,
and an observation $o' \in \mathcal{O}$ is an elementary event.

%%% TODO ensuite c'est pas beau!!!
Suppose now that 
we need to compute,
