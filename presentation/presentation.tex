...
\normalsize
\begin{block}{Onera, System Control $\&$ Flight Dynamics Department}
...
\item conditional probabilities are \textbf{imprecisely known}
...
$\rightarrow$ images in the database representative enough of the reality?\\% (test/train)% $\Rightarrow$ hard extraction of a classifier \\
...
\begin{exampleblock}{}
\begin{itemize}
\item belief space trick: POMDP $\Leftrightarrow$ MDP with \textbf{infinite} space\\% $\mathcal{S}$\\ 
\hspace{2.7cm} $\pi$-POMDP $\Leftrightarrow$ $\pi$-MDP with \textbf{finite} space %$\mathcal{S}$
...
\begin{alertblock}{}
\begin{center}
$\forall s \in \mathcal{S}$, $\pi(s) = 1$ $\Leftrightarrow$ total ignorance about $s$\\
each state possible, \textbf{none necessary}
\end{center}
\end{alertblock}
}
%\visible<3->{
...
\hspace{2.5cm} $\rightarrow$ \textit{human-machine interaction (HMI)}\\ 
\hspace{5.5cm} with \textbf{Sergio Pizziol} 
\item \textbf{advances} in $\pi$-POMDP:\\ 
\hspace{2.5cm} $\rightarrow$ \textit{mixed-observability $\&$ indefinite horizon} 
\item \textbf{simplifying computations}:\\ 
\hspace{2.5cm} $\rightarrow$ \textit{ADD-based solver $\&$ factorization} 
...
\begin{frame}
\frametitle{Proof of optimality under Indefinite Horizon}
...
\item criterion value \textbf{non decreasing} with iterations
\visible<4->{
...
\begin{frame}
\frametitle{Scalability capabilities with Mixed-Observability}
...
\end{figure}
\vspace{-0.2cm}
\textbf{Mixed-Observability} (\textit{Ong et al., 2005}): $s \in {\color{blue} \mathcal{S} = \mathcal{S}_v  \times \mathcal{S}_h} $ \\ 
...
\end{frame}
 ...
 \begin{tikzpicture}[overlay,remember picture]
   \begin{scope}[shift={(current page.south west)}]
	...
	\node at (9,6.9) {when probabilities too imprecise};
   \end{scope}
 \end{tikzpicture}
 }
...
\framesubtitle{\footnotesize $\pi$-MOMDP for robotics with imprecise probabilities}
\vspace{0.4cm}
...
\draw[step=0.2cm] (-0.7,-0.7) grid (1.7,1.7);
\draw (0,0) -- (0,1) -- (1,1) -- (1,0) -- cycle;

\tikzstyle{mvertex}=[circle,fill=blue!35,minimum size=10pt,inner sep=0pt]


\node[mvertex] (t1) at (-0.5,1.5) {$T1$};
\node[mvertex] (t2) at (1.5,-0.5) {$T2$};
%\node (t1G) at (-0.4,1.5) {};
%\node (t2G) at (-0.4,-0.5) {};
%\draw[->,>=latex] (t1) to[bend left] (t1G);
%\draw[->,>=latex] (t2) to[bend right] (t2G);

\tikzstyle{grisEncadre}=[very thick, fill=gray!20] 
\draw [grisEncadre] (-0.65,-0.35) rectangle (-0.35,-0.05);
\node (jacky) at (-0.5,-0.25) {\tiny \textbf{00}}; 
\draw [grisEncadre] (-0.55,-0.35) rectangle (-0.45,-0.55);

\visible<2->{
\draw[color=red,thick] (-0.7,0.5) arc (-90:0:1.2);
\draw[color=red,thick] (1.7,0.5) arc (90:180:1.2);


\node[color=red] (dnode) at (1.9,0.5) {$r$};
\node[color=red,rotate=30] (zoneErr) at (0.5,0.5) {error zone};
\node[color=red] (zoneErr) at (1.7,1.6) {\scriptsize wrong observation};
\node[color=red] (zoneErr) at (1.6,1.3) {\scriptsize with proba $P_{bad}$};
}
\end{tikzpicture}
\hspace{-0.6cm}
%\includegraphics[width=.45\linewidth]{robotgrid.pdf} 
\visible<3->{
\includegraphics[width=.49\linewidth]{courbe1111.png}\\
}
\vspace{0.1cm}
{\color{blue!45} models:} observation accuracy decreases with distance to target\\
\visible<2->{
{\color{red} real model:} takes into account the error zone ({\color{red} $P_{bad} > \frac{1}{2}$})
}
\end{exampleblock}
\visible<4->{
\grille
...
\begin{itemize}
\item factorization $\Rightarrow$ each DP step divided into $n$ stages\\ 
...
\end{exampleblock}
\visible<2->{
\textbf{ Performance, function of the problem index }  \\
\vspace{0.2cm}
\begin{minipage}{0.45\linewidth}
\centering
{\color{orange} reached goal frequency} \\
\includegraphics[scale=0.29]{courbePerfMDP.png} 
\end{minipage}
\hspace{0.6cm}
\begin{minipage}{0.45\linewidth}
...
\begin{tikzpicture} 
\node at (-1,0) {};
\node at (0.85,0) { \color{gggreen} lower is better};
\end{tikzpicture}
\end{minipage}
\hspace{0.6cm}
\begin{minipage}{0.45\linewidth}
...
\begin{tikzpicture} 
\node at (-1,0) {};
\node at (1.65,0) { \color{gggreen} lower is better};
\end{tikzpicture}
\end{minipage}
\begin{alertblock}{}
\centering
...
using poss-prob tranformations {\footnotesize \textit{(Dubois $\&$ Prade 1982)}}% $\textbf{p} \paren{ o' \sachant s,a } = \sum_{s' \in \mathcal{S}} O(s',a,o') \cdot T(s,a,s');$
...
\visible<2->{
\begin{itemize}
\item reward pessimistic according to $\beta$
\end{itemize}
\hspace{0.5cm}
\begin{tikzpicture}[scale=0.3,transform shape]
\definecolor{ggreen}{rgb}{0.3,0.7,0.4};
\begin{axis}[xtick=\empty,ymin=-0.17,ymax=1.1, xmin=-2.2]
\foreach \xx in {-1.5,-1,-0.5,0,0.5,1,1.5} {
\addplot[black,dotted] coordinates {(\xx,0) (\xx,1)};
}

\foreach \yy in {0,0.2,0.4,0.6,0.8,1} {
\addplot[red,dotted,line width=2.5pt] coordinates {(-1.5,\yy) (1.5,\yy)};
}


\addplot[mark=none, blue, line width=2.5pt] coordinates {  (-1.5, 0) (-1, 0) (-1, 0.4) (-0.5,0.4) (-0.5,1) (0,1) (0,1) (0.5,1) (0.5,0.6) (1,0.6)  (1, 0.2) (1.5, 0.2) (1.5, 0) };




\end{axis}
\node (a) at (1.6,0.25) {\Large $s_A$};
\node (b) at (2.5,0.25) {\Large $s_B$};
\node (c) at (3.35,0.25) {\Large $s_C$};
\node (d) at (4.2,0.25) {\Large $s_D$};
\node (e) at (5.05,0.25) {\Large $s_E$};
\node (f) at (5.9,0.25) {\Large $s_F$};
\node (beta) at (5.8,4.3) {\Huge \color{blue} \scalebox{1.5}{$\beta$}};
\node (pi1) at (0.6,5.2) {\Large $\pi_1$};
\node (pi2) at (0.6,4.3) {\Large $\pi_2$};
\node (pi3) at (0.6,3.4) {\Large $\pi_3$};
\node (pi4) at (0.6,2.5) {\Large $\pi_4$};
\node (pi5) at (0.6,1.6) {\Large $\pi_5$};
\node (pi6) at (0.6,0.7) {\Large $\pi_6$};

\node (B1) at (0.6,-0.5) {\color{ggreen} \Large $B_1$};
\node (B2) at (0.6,-1.3) {\color{ggreen} \Large $B_2$};
\node (B3) at (0.6,-2.1) {\color{ggreen} \Large $B_3$};
\node (B5) at (0.6,-2.9) {\color{ggreen} \Large $B_4$};
\node (B6) at (0.6,-3.7) {\color{ggreen} \Large $B_5$};
\node (B6) at (0.6,-4.5) {\color{ggreen} \Large $B_6$};

\draw[black,dotted] (2.85,0.5) -- (2.85,-5);
\draw[black,dotted] (4.55,0.5) -- (4.55,-5);
\draw[black,dotted] (2,0.5) -- (2,-5);
\draw[black,dotted] (1.2,0.5) -- (1.2,-5);
\draw[black,dotted] (5.4,0.5) -- (5.4,-5);
\draw[black,dotted] (6.2,0.5) -- (6.2,-5);

\draw[color=ggreen,thick] (2.9,-0.5) -- (4.55,-0.5);
\draw[color=ggreen,thick] (2.9,-1.3) -- (4.55,-1.3);
\draw[color=ggreen,thick] (2.9,-2.1) -- (5.4,-2.1);
\draw[color=ggreen,thick] (2,-2.9) -- (5.4,-2.9);
\draw[color=ggreen,thick] (2,-3.7) -- (6.2,-3.7);
\draw[color=ggreen,thick] (1.2,-4.5) -- (6.2,-4.5);
\end{tikzpicture}
\begin{tikzpicture}
\definecolor{ggreen}{rgb}{0.3,0.7,0.4};
\node at (0,2.2) {\textbf{Pessimistic Choquet Integral}};
\node at (0,1) {$r(\beta,a) = \displaystyle \sum_{i=1}^{\# \mathcal{L}-1} (\pi_i - \pi_{i+1}) \cdot \min_{ B_i }  r(s,a)$};
\node at (0,0) {\color{ggreen} $B_i = \set{ s \in \mathcal{S} \mbox{ s.t. } \beta(s) \geqslant \pi_i } $};
\end{tikzpicture}
}
...
\node[hvertex] (sh3) at (7.5,5.5) {$s_v^{c}$};
...
\visible<2->{
\item \textbf{advances}: $\rightarrow$ mixed-observability modeling\\
\hspace{1.9cm} $\rightarrow$ indefinite horizon + optimality proof
}
\visible<3->{
