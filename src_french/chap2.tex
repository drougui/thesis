...
les variables du $\pi$-PDM résultant d'un tel $\pi$-PDMOM 
...
\end{subfigure}
\vspace{0.5cm}
\caption[PPUDD vs. APPL et symb-HSVI, problème RockSample]{PPUDD vs. APPL and symb HSVI sur le problème RockSample: 
...
$30$ exécutions par instance et
$18$ minutes par instance: 
elle dure donc $24$ heures au total.

Quatre planificateurs ont été proposés pour cette compétition:
\begin{itemize}
\item \textit{PROST} \cite{KellerE12}, basé sur \textit{Upper Confidence bound applied to Trees} (UCT, \cite{Kocsis:2006BBM2091602.2091633});
\item \textit{GOURMAND} \cite{KolobovMW12,Kolobov12reverseiterative}, based on \textit{Labeled Real Time Dynamic Programming} (LRTDP, \cite{Bonet03labeledrtdp});
\item \textit{symbolic LRTDP} \cite{symbLRTDP};
\item our algorithm PPUDD.
\end{itemize}

Comme le score donné aux planificateurs ne dépend que des $40$ premières
étapes du processus,
la version présentée de PPUDD
est l'algorithme \ref{PPUDD}
avec la ``condition du while'' $\overline{U^*} \neq \overline{U^c}$ 
à la ligne \ref{while_PPUDD}
remplacée par la condition ``itération $\leqslant 40$''.
Il augmente de manière incrémentale
l'horizon de planification
en maintenant un masque stocké sous forme
d'un BDD (arbre de décision binaire, 
\textit{i.e.} un ADD avec des feuilles dans $\set{0,1}$)
...
Bien que PPUDD soit un algorithme de calcul hors-ligne,
nous avons aussi proposé \textit{AnyTime PPUDD} (ATPPUDD),
une version qui gère les temps de calculs 
comme décrit dans \cite{KolobovMW12}. 

La bibliothèque utilisée pour faire les calculs entre ADDs
est \textit{CU Decision Diagram Package} 
(CUDD, \url{http://vlsi.colorado.edu/~fabio/CUDD/}),
et les versions de PPUDD décrites sont à l'adresse 
\url{https://github.com/drougui/ppudd}.

Les figures qui suivent
sont les résultat d'IPPC 2014:
les scores sont données en fonction de l'indice de l'instance,
qui augmente généralement avec la difficulté du problème associé.
...
\end{axis}
\end{tikzpicture}
\caption[Résultats d'IPPC 2014: problèmes \textit{Academic advising} et \textit{Crossing traffic}]{
...
qu'aucune voiture n'arrive a été fixé à $1$
par notre traduction naïve de MDP en $\pi$-MDP,
...
(avec une probabilité $<0.5$ mais assez grande pour être prudent). 
Ceci explique la pauvre qualité des stratégies produites pour ce domaine.
Notons cependant que, pour les $6$ dernières intances (\textit{i.e.} les problèmes les plus difficiles)
notre approche mènent à de meilleures stratégies
que le planificateur probabiliste Symbolic LRTDP.
...
\end{axis}
\end{tikzpicture}
\caption[Résultats d'IPPC 2014: problèmes \textit{Elevators} and \textit{Skill teaching}]{
Résultats de la compétition internationale de planification probabiliste -- session entièrement observable}
\label{figure_IPPC_ELE_SKI}
\end{figure}

Dans le problème \textit{Elevators}, 
des gens arrivent de manière aléatoire 
devant un ascenseur
et doivent être transportés
à l'étage choisi d'un immeuble:
...
PROST et GOURMAND.
Cependant PPUDD et ATPPUDD sont meilleurs que Symbolic LRTDP, 
...
PPUDD et ATPPUDD ont des plutôt bon comportements avec le problème \textit{Skill teaching} 
comme illustré par la même figure.
De plus, ATPPUDD mène à de meilleurs résultats
pour les trois dernières instance:
puisque ces instances 
sont les problèmes du domaine \textit{Skill teaching} 
...
\end{axis}
\end{tikzpicture}
\caption[Résultats d'IPPC 2014: problèmes \textit{Tamarisk} et \textit{Traffic}]{
Résultats de la compétition internationale de planification probabiliste -- session entièrement observable}
...
ne sont même pas exécutés
puisque l'instanciation des ADDs définissant le problème 
prennent trop de temps. 
Symbolic LRTP fait face au mêmes problèmes
puisqu'il utilise aussi des ADDs.
Le domaine \textit{Traffic} est vraiment 
très dur à résoudre avec PPUDD et ATPPUDD (\textit{cf.} figure \ref{figure_IPPC_TAM_TRA}). 
...
ainsi l'instanciation des ADDs en mémoire prennent trop de temps, 
...
xlabel={Problème \textit{Wildfire}},
ylabel={Moyenne brute},
...
\end{axis}
\end{tikzpicture}
\caption[Résultats d'IPPC 2014: problèmes \textit{Triangle tireworld} et \textit{Wildfire}]{
Résultats de la compétition internationale de planification probabiliste -- session entièrement observable}
...
Tout d'abord, ATPPUDD fait face
...
et deux exécutions ont été réalisée
pour les autres instances
(ce qui explique le faible score pour chaque instance).
Comme déjà mentionné pour le domaine \textit{Tamarisk}, 
l'instanciation des ADDs prend trop de temps pour les dernières instances,
et aucune exécution n'est réalisée pour les $4$ dernières instances
avec PPUDD: Symbolic LRTDP rencontre les même problèmes.
Le dernier domaine, appelé \textit{Wildfire}, 
constitue un problème très fréquentiste:
il implique des départs de feux.
C'est pourquoi PPUDD et ATPPUDD ne sont pas vraiment efficaces, 
mais pas trop distant des résultats de Symbolic LRTDP.


\section{Conclusion}
Nous avons présenté PPUDD,
le premier algorithme, à notre connaissance,
que résout les PDM(MO) possibilistes qualitatifs
...
Nos résultats expérimentaux montrent que
l'utilisation d'un algorithme exact (PPUDD) 
...
par des algorithmes procédant à des approximations (APPL)
...
Un autre problème semble être l'utilisation des ADDs:
l'instanciation de ces objets 
avant même le début des calculs
prend un temps trop important,
où bien ne passe pas même pas en mémoire.
Cette difficulté est partagée avec le planificateur Symbolic LRTDP.
Des problèmes de modélisation ont aussi été mis en évidence:
certains problèmes nécessitent une approche optimiste,
et d'autre un approche pessimiste.

Cependant, bien que PROST et GOURMAND ont des performances deux fois supérieures à
PPUDD et ATPPUDD, ces derniers proposent des stratégies 
dont les performances sont deux fois supérieures
à son équivalent probabiliste Symbolic LRTDP.
Notons finalement que PPUDD est disponible dans le dépôt \url{https://github.com/drougui/ppudd}.
