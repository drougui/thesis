...
est aussi naturellement gérée par le formalisme possibiliste qualitatif.

Plus généralement,
les modèles possibilistes qualitatifs
ont besoin de moins d'information à propos du système 
que le modèle probabiliste:
les plausibilités des événements
sont ``seulement'' classifiées
dans l'échelle possibiliste $\mathcal{L}$ plutôt que quantifiées.
...
ou même la mission décrite par la figure \ref{exprobot});
...
\subsection*{Graphical work on independence and factorization (\textit{PPUDD})}
%%%%%%%%%%%%%
%%% chap3 %%%
%%%%%%%%%%%%%
%%THEORETICAL CONTRIBUTIONS
...
called \textit{PPUDD}, is another contribution:
it is the symbolic version of 
the dynamic programming scheme 
proposed in the previous chapter.
Inspired by SPUDD \cite{Hoey99spudd:stochastic}, 
\textit{PPUDD} means \emph{Possibilistic Planning Using Decision Diagrams}. 
...
can be more easily solved by \textit{PPUDD}. 
...
%%%% PRACTICAL CONTRIB
%% WEE!
The motivation leading to the design of \textit{PPUDD},
...
For instance, 
\textit{PPUDD} performances have been compared 
...
\textit{PPUDD} computes a strategy maximizing 
...
(\textit{PPUDD}) for an approximate model ($\pi$-MOMDPs) 
...
and an implementation of \textit{PPUDD} 
...
with adapted versions of \textit{PPUDD}.
...
\textit{PPUDD} and a probabilistic solver 
...
with \textit{PPUDD} in previous experiments 
...
d'IPPC 2014  pourrait être contourné à l'aide d'une discrétisation plus importante:
les version de \textit{PPUDD} pour IPPC 2014 
...
Il serait instructif d'avoir une idée de l'impact de ce prétraitement
sur les performances de \textit{PPUDD}: 
...
les PDM possibilistes qualitatifs
...
Il peut être utilisé pour l'aggrégation de préférences
ou pour calculer un degré de possibilité plus fin d'une trajectoire.
Notons que cela peut être une amélioration utile, 
mais qu'elle complexifie inévitablement le problème.
Des critères plus discriminants sont aussi étudiés dans la littérature
