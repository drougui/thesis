...
Now an obvious remark: if $A$ and $B$ are two subsets of $\Omega$,
$``B \subseteq A'' \Leftrightarrow ``B \cap \overline{A} = \emptyset''$. 
Thus, $d^A = \min \Big\{ i \in \set{1,\ldots,d} \Big\vert F_i \cap \overline{A} = \emptyset \Big\}
= $
...
$\set{ l_1,l_2,\ldots,0 }$:
as the computations allowed 
on the distributions
uses only the operators 
$\max$ and $\min$,
no computation can generate
a value which is not already included 
...
and modeling.
%%%% TODO COMPLEXITY???? poss qual < poss quant / proba TODO
...
Possibility Theory expresses a partial knowledge
...
is given by such a distribution. %%IKI TODO

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\textbf{(pour mon coeur d'amour:) FIN } %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

the  measures This fact results in two measures  POSS/NECC %TODO TODO lien avec criteres
   
As detailed in Section \ref{subsection_qualcrit},
two qualitative criteria have been proposed,
counterpart of EQUATION AU DESSUS
\cite{DBLP:journals/ijar/SabbadinFL98}: 
a pessimistic one,
qualitative counterpart of the maximin, 
or worst-case, criterion in IPT.
An other proposed criterion is the
optimistic, 
counterpart of 
...
it allows a formal modeling of
...
r\'eduisant la complexit\'e du probl\`eme. 
...
finite belief space, managed values are qualitative, complexity

imprecision naturally expressed, 

qualitative possibilistic beliefs can model agent initial ignorance.

however frequentist information cannot be encoded
(SIMPLIFY or ENCODE AVAILABLE DATA)
Imprecision: worst case $\sim$ pessimistic



Cependant, ce mod\`ele n'a \'et\'e
que tr\`es peu \'etudi\'e, et les recherches le concernant peu abouties.\\
bien que ce mod\`ele n'ait pas \'et\'e \'etudi\'e de 
mani\`ere approfondie. \\

PSR \ref{Littman01predictiverepresentations} 
as representation approximated in a more compact way
