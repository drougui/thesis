...
\chapter*{Motivations and introductive considerations}
\addcontentsline{toc}{chapter}{Motivations and technical introduction}
...
whose focal sets are nested (see Figure \ref{figure_focalsets}).
...
and then 
\[ Pl(A \cup B) = \sum_{i=1}^{d_{A\cup B}} m(F_i) = \max \set{ \sum_{i=1}^{d_{A}} m(F_i), \sum_{i=1}^{d_{B}} m(F_i)  } 
...
$\pi(\omega) = \mathds{1}_{\set{ \tilde{\omega} }}(\omega)$:
