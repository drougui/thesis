...
the strategy which is looked for
maximizes the lowest criterion
(among the considered possible parameters).
...
(called \textit{Dynamic Programming}, see Section \ref{chap_SOTA} of Chapter \ref{subsectionDP}).
However, when the parameter imprecision is stationary,
which is more consistent with the presented POMDP definition
(transition and observation functions don't depend
on the time step) 
the proposed computations lead to a approximately optimal strategy,
...
this intuition is quite well as illustrated in Figure \ref{uncertainty_theories}
as the expressiveness of the Imprecise Probability Theory
is on top compared with other theories.
As POMDP solving is already a really hard problem
in terms of complexity as explained above,
using a framework leading to more complex computations
does not seem to be a good approach. 
Moreover, with this framework, 
some restrictive assumptions are commonly needed
such like an imposed form of the credal sets (convex), 
and optimization may require linear programs.
As discussed during this thesis 
(see Section \ref{section_expe_PPUDD} of Chapter \ref{chap_symb}),
it seems quite weird to model a problem in an expressive manner,
and then to allow important approximations without control of them,
as done with the POMDPIP and BPPOMDP frameworks.
It should be a better approach to start with a simpler model
which can be solved in practice. 
...
The reason is that robotic systems may be expensive,
and executions are not allowed to break the robot.
By opposition with on-line (\textit{i.e.} during execution) 
learning problems, the problem of interest here 
...
it is simply a probability distribution
as $bel=Pl$.
...
in order to extract a frequency of 
the exit's location. 
...
making the computed strategy to 
ensure that reached belief states have a low entropy
\textit{i.e.} are as close to deterministic belief states as possible,
while satisfying the requirement of an high expected total reward.
However, a belief state with an high entropy  
may be not related to the agent's lack of knowledge,
but rather to the (fully known) 
variance of the system state:
hence the agent does not know which is the actual system state 
since the belief state is far from being deterministic,
but it may perfectly know how the state behaves,
and then nothing can be improved concerning its knowledge.
% TODO
The use of the entropy supposes then that there is no difference
between the 
be considered as a full knowledge of the
actual frequency of the considered state:
introduce models more general \cite{conf/nips/Araya-LopezBTC10,oatao11437}
