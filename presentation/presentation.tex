...
\node[vertex] (M) at (-1,0) { \color{black!20} -------------};
\node () at (-1.1,0.35) {\textbf{Machine}};
...
\node () at (-1,-0.5) {\includegraphics[scale=0.025]{fcu}};
\node () at (-2.6,-0.1) {\includegraphics[scale=0.025]{manettegaz}};
...
\draw[->,>=latex,line width=2pt,color=red] (4.24,-0.3) -- (-0.05,-0.3);

\draw[->,>=latex,line width=1pt,color=red, dashed] (3.9,-0.3) -- (3.9,-1.3);
\draw[->,>=latex,line width=1pt,color=red, dashed] (1,0.3) -- (1,-1.3);

\node () at (0.9,0.5) {\footnotesize Feedbacks};
\node () at (3,-0.1) {\footnotesize Interface};
\node () at (2.8,-0.5) {\footnotesize manipulations};
\end{tikzpicture}
\vspace{-0.3cm}
...
\visible<2->{
	...
	\hspace{1.5cm} but may lose a feedback $\rightarrow$ $\pi \paren{s_A',o_f' \sachant s_A} = \frac{2}{3}$ 
	\visible<3->{
		\begin{itemize}
			\item  observation: \textbf{human manipulation} $o_m' \in \mathcal{O}$:
		\end{itemize}
		manipulation $o_m'$ normal under $s_A$ $\rightarrow$ $\pi \paren{s_B',o_m' \sachant s_A} = 1$\\
		\hspace{3.6cm} abnormal manipulation \hspace{1.1cm} $ = \frac{1}{3}$ 
	}
	\visible<4->{
		\begin{itemize}
...
\item validated with pilots on a flight simulator missions
\end{itemize}
}
\end{frame}
...
\framesubtitle{\footnotesize undeterminate horizon}
\vspace{0.1cm}
\textbf{contribution (UAI-13): undeterminate Horizon}
\visible<2->{
\vspace{-0.2cm}
...
\hspace{1.3cm} $\rightarrow$ undeterminate horizon + optimality proof
}
\visible<3->{
