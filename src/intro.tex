...
or yet of the transition and observation function:
...
fulfill the mission
as well as possible,
and from the very first execution,
...
and agent knowledge 
suggest us to consider alternative uncertainty theories:
...
and a given testing dataset of pictures (as NORB, see Figure \ref{NORB})
is not obvious.
...
%%%%%%%%%%%%
% POSS QUAL
\subsection*{A possibilistic model}
...
where $d_A = \max_{i \in \set{1,\ldots,d}} F_i \cap A \neq \emptyset$.
As $d_{A \cup B} = \max_{i \in \set{1,\ldots,d}} F_i \cap (A \cup B) \neq \emptyset$,
this integer can be written $d_{A \cup B} = \max \set{d_A,d_B}$, and then
\[ Pl(A \cup B) = \sum_{i=1}^{d_{A\cup B}} m(F_i) = \max \set{ \sum_{i=1}^{d_{A}} m(F_i), \sum_{i=1}^{d_{B}} m(F_i)  } = \max \Big\{ Pl(A), Pl(B) \Big\}. \]
...
ones $\set{ s \in \mathcal{S} \mbox{ s.t. } \pi(\omega)=l_i}$
down to the set of impossible states 
$\set{ s \in \mathcal{S} \mbox{ s.t. } \pi(s)=0  }$.

Note that we can differentiate 
Quantitative Possibility Theory
from Qualitative Possibility Theory.
The former uses the product 
when computing 
the intersection of two events 
$A,B \subseteq \Omega$:
the distribution is 
the product of a possibility distribution 
and a conditional one
(much like in Probability Theory).
The latter uses 
the minimum operator 
for this purpose
(see Section \ref{posspres} 
of Chapter \ref{chap_SOTA}).
Thus, the qualitative theory 
uses only $\max$ and $\min$ operators:
%%%% TODO CA CHIE JUSTE APRES: qual neq quant
whatever possibility values defining
managed
in this framework after 
the use of the operators 
$\max$ and $\min$
remains in the set 
,
\textit{i.e.} if a possibility value
has not been initially introduced in
the possibility values introduced
when defining the global
This is not true for 
Quantitative Possibility Theory
since the use of the product operator 
leads to the creation 
of new real values in $[0,1]$. 
%%%TODO if successive distributions about new variable are used 
In the qualitative framework, 
the (real) possibility values 
assigned to the elementary events 
are not really taken into account
since only qualitative comparisons 
are performed via the use of 
$\max$ and $\min$ operators.
Thus, Qualitative Possibility Theory
is classically defined 
with the introduction of
a qualitative scale $\mathcal{L}$:
it may be defined as $\set{l_1,l_2,\ldots,0 }$,
or as any other totally ordered scale indifferently.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{(pour mon coeur d'amour:) FIN } %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


Algebraic structures different from the linear algebra:
semirings $(\mathcal{L},\max,\min)$, $([0,1]\max,\times)$.
close to the well known 
tropical semirings \cite{pin:hal-00113779},
as the \textit{max-plus} semiring $(\mathbb{R} \cup \set{-\infty} \max,+)$ 
and the \textit{min-plus} one $(\mathbb{R} \cup \set{+\infty}, \min,+)$.
(SIMPLIFY or ENCODE AVAILABLE DATA)
...
and model the lack of knowledge.
benefiting from the lack of knowledge
...
Finding qualitative estimates of their recognition performance is easier: 
the $\pi$-POMDP model only require qualitative data, thus it allows to
...
with a model built from imprecise data
...
PSR \ref{Littman01predictiverepresentations} as representation approximated in a more compact way

 De plus, ce cadre possibiliste 
...
r\'eduisant la complexit\'e du probl\`eme. managed values are qualitative
As number of belief finite, and MDP P complexity \cite{DBLP:journals/corr/abs-1202-3718},
at most exponential the process description. (instead of undecidable!)
...
The goal of this thesis is to determine
what the Qualitative Possibility Theory can bring 
in Planning under Uncertainty 
and more generaly Sequential uncertainty management in practice, 
