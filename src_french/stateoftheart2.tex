...
cependant ce modèle résume de manière appropriée les principales caractéristiques d'un système robotique:
...
\section{Théorie des Possibilités Qualitatives}
\label{posspres}
...
\textit{i.e.} une mesure de possibilité quantitative $\Pi$ représente un ensemble particulier
...
qui s'écrit $\Pi(A) \leqslant \Pi(B)$.
...
et possédant un ordre total. 
Dans ce travail, 
l'échelle possibiliste est définie par $\mathcal{L} = \set{ 0, \frac{1}{k}, \ldots, 1 }$
pour simplifier les notations.

Nous pouvons définir la \textit{distribution de possibilité} $\pi$
comme suit: $\forall \omega \in \Omega$, $\pi(\omega) = \Pi(\set{\omega})$.
D'après la définition \ref{poss_measure},
une mesure de possibilité $\Pi$ est entièrement 
définie par la distribution associée $\pi$.
...
\label{DEF_necessity}
La mesure de nécessité associée à la mesure de possibilité $\Pi$ est la mesure floue $\mathcal{N}:2^\Omega \rightarrow \mathcal{L}$ 
telle que $\forall A \subset \Omega$, \[ \mathcal{N}(A)=1-\Pi(\overline{A}), \]
où $\overline{A}$ est l'événement complémentaire de $A$ dans $\Omega$. 
\end{Def}
}
\begin{figure} 
\center
...
\end{tikzpicture}
\caption[Distribution de possibilité, mesure de nécessité et spécificité]{
...
comme illustré
par les flèches oranges en pointillées. }  
...
aucun événement n'est nécessaire, sauf l'univers entier $\Omega$.

La connaissance parfaite que l'événement élémentaire $\omega_A \in \Omega$
est elle modélisée par une distribution de possibilité $\pi$
telle que $\pi(\omega_A) = 1$
et $\pi(\omega)=0$, $\forall \omega \neq \omega_A$. 
Le degré de nécessité du singleton $\set{\omega_A}$
est aussi égal à $1$: $\mathcal{N}(\set{\omega_A})=1-\Pi(\overline{\set{\omega_A }}) = 1$. 
L'événement élémentaire $\omega_A$
est nécessaire, et tous les autres événements élémentaires ont un degré de nécessité nul:
si $\omega_B \neq \omega_A$, 
...
\end{Def}
Les notions de nécessité et de spécificité sont illustrées figure \ref{figure_necspec}.

Les concepts de la théorie des possibilités qualitatives nécessaires à la compréhension de notre travail ont été présentés.
Nous présentons maintenant l'homologue du critère basé sur l'espérance dans les modèles probabilistes:
les critères qualitatifs utilisés dans les modèles possibilistes qualitatifs.
...
est simplement l'intégrale de la la fonction de récompense
contre la mesure de probabilité.
Le concept d'intégrale a été étendue aux mesure floues:
quand la mesure est quantitative, 
l'extension est appelée \textit{intégrale de Choquet} \cite{Choquet1954}.
Dans le cas des mesures qualitatives, 
l'objet résultant est l'\textit{intégrale de Sugeno} \cite{Sugeno74}.
Nous définissons donc ici l'intégrale de Sugeno:
\begin{Def}[Intégrale de Sugeno]
...
tel que la mesure $\mu$ de $\set{ \omega \sachant f(\omega) \geqslant \lambda}$ 
est plus grande ou égale à $\lambda$. 
Par exemple, le $h$-indice
(ou indice de Hirsh)
est l'intégrale de Sugeno de la fonction $papier \mapsto \# citations$
...
mesurant la validité des actions étant donné un modèle possibiliste et une fonction de préférence,
a été proposé dans \cite{SabbadinFL98, DuboisPS01,Dubois95possibilitytheory},
basé sur les intégrales de Sugeno (\ref{equation_sugenoposs}) et (\ref{equation_sugenonec}).
Rappelons que l'ensemble $\mathcal{S}$ (resp. $\mathcal{A}$) 
est comme dans l'introduction 
l'ensemble fini des états du système $s$ (resp. des actions $a$).
...
l'intégrale de Sugeno de la préférence contre la mesure de possibilité $\Pi_a$
...
l'intégrale de Sugeno de la préférence 
contre la mesure de nécessité associée à $\Pi_a$, 
notée $\mathcal{N}_a$, 
...
Le critère pessimiste (\ref{equation_critpess}) cherche à éviter les états non désirés,
...
et $\mathcal{A} = \set{ a_1 , a_2 }$ l'ensemble des actions. 
...
le système est dans l'état $s_B$ de manière déterministe (avec nécessité $1$).
\end{itemize}
...
\end{tikzpicture}
\caption[Exemple d'une situation pour illustrer les critères qualitatifs]{Illustration de l'exemple de la section \ref{subsection_qualcrit}
à propos des critères qualitatifs. L'action $a_1$ maximise le critère optimiste (\ref{equation_critopt}),
...
puisque le pire état n'est pas atteignable avec cette action.}  \label{figure_example}
\end{figure}
...
entièrement possible, donc l'état $s_A$ n'est pas du tout nécessaire: 
une action plus prudente est $a_2$, avec laquelle l'état devient $s_B$
avec certitude, mais avec une plus petite préférence.
On peut vérifier facilement que l'action $a_2$ maximise bien le critère pessimiste (\ref{equation_critpess}):

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Processus Décisionnel Markovien Possibiliste Qualitatif ($\pi$-PDM)}
\label{subsection_piMDPs}
Un \textit{processus décisionnel markovien possibiliste qualitatif}, ou $\pi$-PDM,
présenté dans \cite{Sabbadin2001287,Sabbadi00,Sabbadin1999pipomdp}, 
...
avec $S_t \in \mathcal{S}$ $\forall t \geqslant 0$, sont markovien.
...
la variable $S_{t+1}$ est indépendante (au sens causal) des variables $\set{ S_0, \ldots, S_{t-1} }$,
...
à partir des critères possibilistes (\ref{equation_critopt}) et (\ref{equation_critpess}), 
nous introduisons, pour un horizon $H\geqslant0$, 
les \textit{trajectoires de longueur $H$}, 
$\mathcal{T} = (s_1,\ldots,s_{H})$,
...
\end{align}
C'est équivalent au critère optimiste (\ref{equation_critopt}),
...
la \textit{stratégie optimale pessimiste} $\underline{\delta^*}$ 
...
et la \textit{fonction valeur pessimiste optimale}
 est la fonction valeur maximale en faisant varier la stratégie $(\delta) \in \Delta_H$:
\\
\\
\vbox{
...
$\forall s \in \mathcal{S}$,
\begin{align} 
\label{equation_pessqualvaluestar} \underline{U_H^*}(s) &= \max_{(\delta) \in \Delta_H} \set{ \underline{U_H} \Big(s,(\delta)\Big) } & \mbox{ (fonction valeur optimale pessimiste), } \\
\label{equation_pessqualstratstar} \underline{\delta^*}(s) &\in \operatorname*{argmax}_{(\delta) \in \Delta_H} \set{ \underline{U_H} \Big(s,(\delta)\Big) } & \mbox{ (stratégie optimale pessimiste). } 
\end{align}
\end{Def}
}
...
peuvent être calculés comme suit: $\forall s \in \mathcal{S}$,
\begin{align}
\nonumber 
...
\textit{i.e.} dans le cas où les fonctions de préférence $\rho_t$ ne sont pas prises en compte:
dans ce cas, seule la fonction de préférence terminale $\Psi$
modélise le but de la mission.
Enfin, on remarque qu'il suffit de considérer un $\pi$-PDM classique
avec $\rho_t(s,a) = 1$, $\forall s \in \mathcal{S}$, $\forall a \in \mathcal{A}$ et $\forall t \in \set{0,\ldots,H-1}$. 

\section{PDM Partiellement Observable Possibiliste Qualitatif ($\pi$-PDMPO)}
...
représentées par le processus d'observation $(O_t)_{t \in \mathbb{N}}$.
...
Les lignes vertes en pointillé représentent des influences déterministes.}
\label{POMDP} 
\end{figure}
...
Cet ensemble est noté $\Pi^{\mathcal{S}}_{\mathcal{L}} = \set{ \pi: \mathcal{S} \rightarrow \mathcal{L} \sachant \max_{s \in \mathcal{S}} \pi(s) = 1 }$. 
...
$\textit{i.e.}$ le nombre total de fonctions, moins le nombre de fonctions non normalisées.
...
\item $O^{\pi}$, l'ensemble des distributions de possibilité d'observation $\pi \paren{ o' \sachant s',a }$;
\item $\Psi$ la fonction de préférence, définissant pour chaque état $s \in \mathcal{S}$,
la préférence associée à la situation où l'état du système est $s \in \mathcal{S}$;
\item $\beta_0$, \textit{l'état de croyance possibiliste initial},
est la distribution de possibilité définissant l'incertitude
...
'état de croyance initial $\beta_0 \in \Pi^{\mathcal{S}}_{\mathcal{L}}$ 
...
conditionnellement à toutes les données visibles par l'agent. 
...
est l'information visible par l'agent à l'étape de temps $t$ ( $i_0 = \set{} = \emptyset$ ),
...
Si l'état de croyance à l'étape de temps $t$ est $\beta_t$,
l'action choisie est $a_t \in \mathcal{A}$,
et l'état suivant est $o_{t+1}$, 
l'état suivant $\beta_{t+1}$
...
$B^{\pi}_0$ est déterministe égal à $\beta_0$
...
mais avec une préférence terminale $\Psi$ uniquement.
...
et à atteindre un état satisfaisant.
...
Le $\pi$-PDM $\langle \tilde{\mathcal{S}}^{\pi}, \mathcal{A}, \tilde{T^{\pi}}, \underline{\Psi} \rangle$
...
qui est l'application directe du schéma de programmation dynamique optimiste
pour $\pi$-PDM (théorème \ref{DPpiMDP})
avec seulement une préférence terminale (qui est pessimiste):
...
$\overline{U^*_0} \gets \underline{\Psi}$;\\
...
qui est la programmation dynamique pessimiste pour $\pi$-PDM 
(théorème \ref{DPpiMDP}),
...
ainsi que les modèle $\pi$-PDM et $\pi$-PDMPO ont été présentés.
...
tandis qu'elle est définie par des distributions de possibilité $\pi \paren{ s' \sachant s,a } \in \mathcal{L} = \set{ 0, \frac{1}{k}, \ldots, 1}$ (avec $k\geqslant1$) 
...
tandis que le cadre possibiliste qualitatif utilise des préférences $\rho(s,a) \in \mathcal{L}$ et $\Psi(s) \in \mathcal{L}$.
...
Un PDMPO (resp. $\pi$-PDMPO), est redéfini en termes de PDM (resp. $\pi$-PDM) entièrement observable
...
La propriété d'observabilité mixte est définie:
comme pour le modèle probabiliste,
la complexité de résolution des problèmes ayant cette propriété est réduite.
Enfin, l'homologue du problème à horizon infini est formellement défini,
