...
is that no appropriate choice is made,
at the earliest stages prior to computations,
concerning how to manage the parameter imprecision:
for instance, suitable approaches may to be
cautious regarding the imprecision \textit{i.e.} 
taking into account the worst case,
or yet optimistic, considering the best one.

A more recent approach deals 
with the cautious point of view,
and is thus called \textit{robust POMDP} \cite{DBLP:conf/icml/Osogami15}.
Inspired by the corresponding work in the fully observable case 
(called \textit{uncertain MDP} \cite{NE:05}),
this work uses the well-known \ 
Classically, the approach considers, maximin), 
(non stationnary, possibly one model for each time step),
robust  it answers to the robust question (approx), but at least the POMDP complexity,
...
non stationnary uncertain MDP models, or of the robust approximation for stationnary uncertain MDPs 
...
given the imprecision, instead of using a \\  
