...
pictures too different from the pictures from the dataset. 
...
or, at least, good performances are harder to reach.

A confusion matrix can be computed (see Figure \ref{confusion_matrix})
is also computed using such a labeled picture dataset,
...
\textbf{JE T'AIME MA LAURANNE D'AMOUR}
% agent's 
%initial belief and of the transition and 
...
preprocessing followed by classifiers such as the one computed in Figure \ref{CV_algoConvNet} etc.) 
is so complex that probabilities of occurrence are hard to rigorously extract.
...
% CONCL
As a conclusion, the POMDP model supposes the knowledge of all
the involved probabilities:
unlikely these frequencies are imprecisely known in practice.
