...
\normalsize
\begin{block}{Onera, DCSD}
Automatics, AI, Flight Mechanics, Cognitive Sciences
\end{block}
\vspace{0.5cm}
\visible<2->{
...
\begin{itemize}
\item autonomy, steering architectures and\\ 
human factors
\item decision making, planning
\item experimental/industrial applications:
UAVs,\\ 
\hspace{0.9cm} orbital systems,\\
\hspace{0.9cm} exploration robots
\end{itemize}
\end{minipage}
...
\hspace{4cm}(\textit{Papadimitriou et al. 1987})
\end{itemize}
\begin{itemize}
...
in infinite horizon -- {\footnotesize \textit{Madani et al. (AAAI-99)}}
\end{itemize}
\visible<2->{
...
\node at (3,1.15) {\footnotesize less plausible};
\node at (3,0.75) {\vdots};
\node at (3,0.15) {\footnotesize very rare};
...
\begin{exampleblock}{}
\begin{itemize}
\item belief space trick: POMDP $\rightarrow$ MDP with \textbf{infinite} $\mathcal{S}$\\ 
\hspace{2.7cm} $\pi$-POMDP $\rightarrow$ $\pi$-MDP with \textbf{finite} $\mathcal{S}$
...
\begin{block}{}
\begin{itemize}
\item the update \textbf{only depends on $o'$, $a$ and $b_t^{\pi}$}
\end{itemize}
\end{block}
}
\end{frame}
...
\begin{enumerate}
\item introduction
\item natural use of a qualitative possibilistic model
\item updates and first use of the $\pi$-POMDP model 
...
\section{Introductory example (HMI)}

\begin{frame}
\frametitle{Example: Human-Machine Interaction}
\framesubtitle{\footnotesize joint work with Sergio Pizziol -- Context}
\vspace{-0.1cm}
\begin{figure}
\centering
\begin{tikzpicture}[scale=0.8,transform shape]
...
\hspace{2cm} $\rightarrow$ accident
\end{block}
}
\visible<3->{
...
\item \textbf{observation space} $\mathcal{O}$: feedbacks/human selections
\end{itemize}
\end{alertblock}
}
\end{frame}
...
\end{alertblock}
\visible<2->{
	\begin{exampleblock}{ Machine state $A \rightarrow B$}
	\begin{itemize}
		\item \textbf{machine feedback} observation $o_f \in \mathcal{O}$: 
	\end{itemize}
	human usually aware of feedbacks $\rightarrow$ $\pi \paren{s_B',o_f' \sachant s_A} = 1$\\
	\hspace{1.5cm} but may lost a feedback $\rightarrow$ $\pi \paren{s_A',o_f' \sachant s_A} = \frac{2}{3}$ 
	\visible<3->{
		\begin{itemize}
			\item \textbf{human selection} observation $o_s \in \mathcal{O}$:
		\end{itemize}
		selection $o_s'$ normal under $s_A$ $\rightarrow$ $\pi \paren{s_B',o_s' \sachant s_A} = 1$\\
		\hspace{3.6cm} anormal selection \hspace{1.1cm} $ = \frac{1}{3}$ 
	}
	\visible<4->{
		\begin{itemize}
...
\framesubtitle{\footnotesize diagnosis tool for Human-Machine interaction (with Sergio Pizziol)}
\begin{tikzpicture}[scale=0.8,transform shape]
...
\node (Oc) at (0.8,3.7) {feedbacks/selections};
\node (V) at (1.1,3.1) {(observable) \huge $\{$};
...
$\Leftrightarrow$ \textbf{possibilistic belief state}% over $s \in \mathcal{S}$ 
\item \textbf{detection} of human assessment errors
\item \textbf{diagnosis} using \textit{leximin} operator
\item results on flight simulator missions with pilots
\end{itemize}
}
\end{frame}
...
%\nico{	
\section{Updates of the qualitative possibilistic model}%Mixed-Observability and unbounded mission durations}
\begin{frame}
\frametitle{Mixed-Observability (MOMDP) -- {\footnotesize \textit{Ong et al. (RSS-05)}}}
\framesubtitle{\footnotesize $\pi$-Mixed-Observable Markov Decision Process ($\pi$-MOMDP)}
\textbf{contribution (UAI-13):}
\vspace{-0.6cm}
...
\framesubtitle{\footnotesize undefined horizon}
\vspace{0.1cm}
\textbf{contribution (UAI-13): Undefined Horizon}
\visible<2->{
\vspace{-0.2cm}
\begin{block}{Dynamic Programming scheme: $\#$ iterations $<\# \mathcal{X}$}
\begin{itemize}
\item assumption: $\exists$ artificial ``stay'' action\\ 
as in classical planning/ $\gamma$ counterpart 
\item value function $=$ criterion: non decreasing with horizon
\visible<3->{
\item action update for states increasing the value function
...
\section{Symbolic solver and factorization}
\begin{frame}
...
Boolean variables ($X_1,\ldots,X_n$) \\ 
	...
	\vspace{0.3cm}
	probabilistic case:\\ 
	SPUDD, { \footnotesize \textit{Hoey et al., UAI-99}}
	\end{itemize}
	\end{minipage}
	\hspace{0.2cm}
...
\begin{itemize}
\item factorization $\Rightarrow$ DP steps divided into $n$ stages\\ 
$\rightarrow$ used ADDs smaller $\Rightarrow$ \textbf{faster computations}
}
\visible<3->{
...
\hspace{3cm} { \color{red} $\beta_i \in \Pi^{\mathcal{S}_{h,i}}_{\mathcal{L}}$, belief over $s_{h,i}$}
\vspace{-0.1cm}
}
\begin{tikzpicture}
\node at (-2.2cm,0cm) {};
...
\centering
\begin{exampleblock}{}
PPUDD vs SPUDD { \footnotesize \textit{Hoey et al.}} \\ 
{ \color{red!70} Navigation benchmark}: reach a goal -- spots with accident risk \\ 
M1 (resp. M2) optimistic (resp. cautious) criterion 	
\end{exampleblock}
\visible<2->{
...
winners of IPPC 2014, $2 \times$ better than PPUDD \\
\item choice of the qualitative criterion (optimistic/pessimistic)
\item non additive utility degrees\\ 
same scale as possibility degrees (commensurability)
\item frequentist information lost
\end{itemize}
\end{alertblock}
}
\end{frame}
...
using poss-prob tranformations {\footnotesize (Dubois et al., FSS-92)}% $\textbf{p} \paren{ o' \sachant s,a } = \sum_{s' \in \mathcal{S}} O(s',a,o') \cdot T(s,a,s');$
...
\end{itemize}
\begin{block}{}
\[ \Rightarrow \textbf{p} \Big( \beta' \Big\vert \beta,a \Big) = \sum_{ \substack{ o' \mbox{ \footnotesize t.q. } \\ update(\beta,a,o')=\beta'} }\textbf{p}\paren{o'\sachant \beta,a}\]
\end{block}
%\vspace{0.5cm}
...
\framesubtitle{\footnotesize translation summary \textbf{contribution (SUM-15):}}
\vspace{0.3cm}
...
\vspace{1cm}
\begin{itemize}
\item $\min$ of marginals $=$ \textbf{less informative} belief state
...
\begin{block}{}
\begin{itemize}
\item udates: $\rightarrow$ mixed-observability modeling\\
\hspace{1.3cm} $\rightarrow$ undefined horizon 
\item modeling: $\rightarrow$ human-machine interaction \\
$\rightarrow$ robust recognition mission with possibilistic beliefs
\item computations: factorization work $\&$ PPUDD algorithm (competitive solver, IPPC 2014) 
\end{itemize}
\end{block}
\visible<2->{
\begin{exampleblock}{}
new refined criteria $\rightarrow$ finer $\pi$-POMDP 
\end{exampleblock}
}
\visible<3->{
\begin{alertblock}{}
...
\visible<3->{ 
\item factored POMDP $\xrightarrow{ \mbox{\footnotesize \textbf{translation}} }$ \textbf{factored} MPD.
}
\end{itemize}
\end{block}
\visible<4->{
