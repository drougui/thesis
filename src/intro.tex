...
POMDPs with Imprecise Probabilities (POMDPIP) \cite{Itoh2007453} second order belief (imprecision and frequency are confused, no position taken): 
no criterion asserting how imprecision is taken into account, belief discretization using all the possible models,analysis??\\

 Bounded Parameters POMDPs (BPPOMDP) \cite{NiYaLiaZhi}  \\
...
autant travailler sur un already discretized model \\  
...
\begin{figure} \centering
\begin{tikzpicture}
\node (IP) at (2,10) {Imprecise Probability Theory \cite{Walley2000125}};
\node (BF) at (2,7) {Belief Functions Theory \cite{dempster1967,shafer1976mathematical}};
\node (PROB) at (-1,4) {Probability Theory};
\node (POSS) at (5,4) {Possibility Theory \cite{Du2006.7,Dubois1997359}};
...
\node (topr) at (8.5,10) {};
\node (botr) at (8.5,4) {};
\node (middr) at (8,7) [rotate=90] {expressiveness};
\draw[->,>=latex,thick, color=red!60, line width=2mm] (botr) to (topr);
\draw (-6,7) rectangle (-2,10);
...
\node (bluelegend) at (-4,8.5) {particular  \hspace{0.5cm}   case of};
\end{tikzpicture}
...
to represent uncertainty of events and knowledge of it: 
...
it is a possibility distribution.
