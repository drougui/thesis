...
which uses the product when 
combining possibility distributions
(much like Probability Theory),
and Qualitative Possibility Theory
using the minimum operator for this purpose
(see Section \ref{posspres} of Chapter \ref{chap_SOTA}).
Thus, the latter theory uses only $\max$ and $\min$
operators: 
redefining $\set{l_1,l_2,\ldots,0}$
as the values of all the possibility distributions
describing a given situation,
possibility values handled 
in this framework after 
the use of the operators $\max$ and $\min$
remains in the set $\set{l_1,l_2,\ldots,0}$.
This is not true for Quantitative Possibility Theory
since the use of the product operator 
leads to the creation of new real values in $[0,1]$. 
...
are performed through $\max$ and $\min$ operators.
Thus, a qualitative scale $\mathcal{L}$ is classically introduced:
